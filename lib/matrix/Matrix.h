...
template <class numtype>
double Matrix<numtype>::eps = numeric_limits<double>::epsilon();
...
Matrix<numtype> Matrix<numtype>::inverse() const {
	...
	for(i = 0; i < ROWS; i++) {
		for(j = 0; j < COLS; j++) {
			...
			if(fabs(ret.m_matrix[j*COLS+i]) < eps) {
...
void Matrix<numtype>::normalize(bool direction) {
	...
	if(direction == 1) {
		...
		for(int i = 0; i < ROWS; i++) {
			for(int j = 0; j < COLS; j++) {
				m_matrix[i*COLS+j] /= (eps + temp.m_matrix[j]);
			}
		}
	}
	else {
		...
		for(int i = 0; i < ROWS; i++) {
			for(int j = 0; j < COLS; j++) {
				m_matrix[i*COLS+j] /= (eps + temp.m_matrix[i]);
			}
		}
	}
}
...
template <class numtype>
numtype Matrix<numtype>::max(int &indx) const {
	int i, n = ROWS*COLS;
	numtype maxValue = m_matrix[0];
	indx = 0;
	for(i = 1; i < n; i++) {
		if(m_matrix[i] > maxValue) {
			maxValue = m_matrix[i];
			indx = i;
		}
	}
...
inline Matrix<numtype> Matrix<numtype>::dotDivide(const Matrix<numtype> &mat) const {
	...
	int n = ROWS*COLS;
	for(int i = 0; i < n; i++) {
		ret.m_matrix[i] = m_matrix[i]/(eps+mat.m_matrix[i]);
	}
	return ret;
}
template <class numtype>
...
inline Matrix<numtype> Matrix<numtype>::operator*(const Matrix<numtype> &mat) const {
	...
	for(int i = 0; i < ROWS; i++) {
		for(int j = 0; j < mat.COLS; j++) {
			...
			if(fabs(ret.m_matrix[i*mat.COLS+j]) < eps) {
...
inline Matrix<numtype> Matrix<numtype>::operator/(numtype a) const {
	...
	int n = ROWS*COLS;
	for(int i = 0; i < n; i++) {
		ret.m_matrix[i] = m_matrix[i]/(eps+a);
	}
	return ret;
}
